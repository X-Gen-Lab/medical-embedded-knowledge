...
| 类型 | 说明 | 示例 |
|------|------|------|
| `add` | 添加新内容 | `add: add medical device safety standards chapter` |
| `fix` | 修复问题 | `fix: fix broken links in regulatory docs` |
| `update` | 更新现有内容 | `update: update IEC 62304 compliance examples` |
...
```
add: add embedded systems safety guidelines
```

```
fix: fix syntax error in code example
```

```
update: update medical device testing procedures
...
```
add: add some content about medical embedded systems including safety standards, regulatory compliance and other detailed explanations
```
> 描述过长，应该简化
...
```bash
# 分别提交不同类型的修改
git add docs/safety-standards.md
git commit -m "update: update safety standards guide"

git add docs/regulatory-compliance.md
git commit -m "add: add regulatory compliance examples"
```
...
```
add: add medical device risk management chapter
```
...
```
update: update IEC 62304 compliance guide
```
...
```
add: add embedded systems security guide
```
